...
- Front : écran Réglages IA en liste de radios-cartes (IA par défaut + une carte par configuration, cocher bascule aussitôt, Modifier/Supprimer par carte) avec éditeur de création/modification (nom après le champ clé), signal `AiCredentialsService` en enveloppe (`create`/`update`/`remove`/`activate`), pied du chat affichant le nom de la configuration active avec un menu rapide de bascule dans l'engrenage (puis « Gérer les configurations… » vers la modale) ; les sélecteurs de raisonnement du pied écrivent sur l'active.
