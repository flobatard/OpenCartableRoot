...
- Conséquences : ajouter un provider capable = deux frozensets (back et front) + un helper d'encodage ; un modèle inconnu du profil Anthropic est traité comme récent (adaptive) — la lib ne valide la forme de `thinking` qu'à la requête, un mauvais palier serait un 503 en plein flux ; « coupé » n'est pas proposé pour Gemini 3 ni 2.5 Pro (non désactivables ; posé par l'API, il vaut `low`/`thinking_budget 0`) ni pour les modèles OpenAI sans « none » (o-series, gpt-5) ; le catalogue est une dette d'entretien à chaque nouvelle famille de modèles (TODO.md) ; deux écrivains du même credential (formulaire et pied du chat) sont réconciliés par le signal d'`AiCredentialsService` — un formulaire non modifié se réaligne ; un changement au pied entre un `interrupt` et la décision ne s'applique pas à la reprise (`pending.config` réutilisé) ; un `AI_REASONING*` invalide ou hors capacités rend le fallback inutilisable (422 à la résolution, jamais un crash au boot) et n'est pas affiché au front (TODO.md).
